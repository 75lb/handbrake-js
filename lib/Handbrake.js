...
/**
@class
@classdesc A thin wrapper on the handbrakeCLI child_process handle. An instance of this class is returned by `hbjs.spawn()`.
...
Handbrake.prototype.exec = function(options, done){
    var cmd = util.format("\"%s\" %s", this.HandbrakeCLIPath, toSpawnArgs(options, { quote: true }).join(" "));
    cp.exec(cmd, done);
};
