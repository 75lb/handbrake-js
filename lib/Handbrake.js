...
Handbrake.prototype._run = function () {
  ...
  cliOptions.forEach(function(cliOption){
    if (cliOption.needsEqualsSign && cliOption.name != undefined){
      ...
      if (this.options[cliOption.name] && this.options[cliOption.name] !== true){
        ...
        var optionValue = this.options[cliOption.name];   //this really needs to be escaped for quotes...
        var optionNameEqualsValue = cliOption.name + '=' + quote + optionValue + quote;
        this.options[optionNameEqualsValue] = '';     
        delete this.options[cliOption.name];   // prevents duplicate option being sent
      }  
    } 
  },this);
