...
Handbrake.prototype._run = function(){
    ...
    if (this.options.input !== undefined && this.options.output !== undefined){
        ...
        if (pathsEqual){
            err.name = Handbrake.eError.VALIDATION;
    ...
    handle.on("exit", function(code){
        if (code === 0){
            ...
            err = new Error();
            err.name = Handbrake.eError.INVALID_INPUT;
            err.message = "Invalid input, not a video file [error code: " + code + "]";
            self._emitError(err);

        } else if (code === 3){
            err = new Error();
            err.name = Handbrake.eError.OTHER;
    ...
    handle.on("error", function (spawnError){
        ...
        if (spawnError.code === "ENOENT"){
            err.name = Handbrake.eError.NOT_FOUND;
...
/**
Fired on successful completion of an encoding task. Always follows a {@link module:handbrake-js~Handbrake#begin} event, with some {@link module:handbrake-js~Handbrake#progress} in between.
...
/**
All operational exceptions are emitted via the {@link module:handbrake-js~Handbrake#error} event.
@enum
@memberof module:handbrake-js
@inner
*/
