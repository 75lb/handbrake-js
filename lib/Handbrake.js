...
function Handbrake(mocks){
    ...
    this.HandbrakeCLIPath = process.platform === "darwin"
        ? path.join(__dirname, "../bin/HandbrakeCLI")
        : path.join(__dirname, "..", "bin", "HandbrakeCLI.exe");

    if (mocks){
        cp = mocks.cp || origCp;
        this.HandbrakeCLIPath = mocks.HandbrakeCLIPath || this.HandbrakeCLIPath;
    }
}
...
Handbrake.prototype.run = function(){
    ...
    var handle = cp.spawn(this.HandbrakeCLIPath, handbrakeOptions.toArray());
