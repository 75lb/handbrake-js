        ...
        lines.forEach(function(line){
            if (progress.pattern.test(line)){
                ...
                */
                ...
                progressBuffer = progressBuffer.replace(progress.pattern, "");
            }
        });
        done();
    };
...
HandbrakeProcess.prototype.run = function(options){
    ...
    /**
    Spawn HandbrakeCLI, piping both stdout and stderr to an aggregate stream.
    When encoding (an --input has been supplied) all text on stdout regards progress,
    so pipe it through the progressParser
    */
    var handle = cp.spawn(_HandbrakeCLIPath, config.toArray());
    handle.stderr.pipe(this.outputStream, { end: false });
    if (config.input){
        handle.stdout.pipe(this.progressParser);
    } else {
