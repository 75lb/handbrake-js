...
function HandbrakeProcess(){
    ...
    var progressRe = /\rEncoding: task (\d) of (\d), ((.+?), )?(.*) %( \((.*?) fps, avg (.*?) fps, ETA (.*?)\))?/,
        progressBuffer = "",
        self = this;
    ...
    this.progressParser = new stream.Transform();
    this.progressParser._transform = function(chunk, encoding, done){
        var chunkString = chunk.toString();
        self.outputLog += chunkString;
        progressBuffer += chunkString;

        var inEncoding = false;

        var match = progressBuffer.match(progressRe);
        if (match){
            var progress = {
                taskNumber: match[1],
                taskCount: match[2],
            ...
            */
            ...
            progressBuffer = "";
            chunkString = chunkString.replace(progressRe, "");
...
HandbrakeProcess.prototype.run = function(options){
    ...
    handle.stderr.pipe(this.outputStream, { end: false });
    if (config.input){
        handle.stdout.pipe(this.progressParser).pipe(this.outputStream);
    } else {
        handle.stdout.pipe(this.outputStream, { end: false });
    }
    ...
    handle.on("exit", function(code, signal){
        ...
        } else if (code === 0 && /No title found\./.test(self.outputLog)){
