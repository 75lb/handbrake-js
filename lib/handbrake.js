...
HandbrakeProcess.prototype.run = function(options){
    ...
    */
    ...
    handle.stderr.pipe(this.infoTransform, { end: false }).pipe(this.outputStream);
    if (config.input){
        handle.stdout.pipe(this.progressTransform).pipe(this.outputStream);
    } else {
        handle.stdout.pipe(this.infoTransform, { end: false }).pipe(this.outputStream);
    }
