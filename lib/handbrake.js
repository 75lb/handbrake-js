...
HandbrakeProcess.prototype.run = function(options){
    ...
    handle.stdout.on("readable", function(){
        var data = handle.stdout.read();
        if (data){
            outputLog += data;
            dataBuffer += data;
    ...
    handle.stderr.on("readable", function(){
        var data = handle.stderr.read();
        if (data){
            outputLog += data;
            data = data.replace(/\n$/, "");
            self.emit("output", data);
    ...
    handle.on("exit", function(code, signal){
        ...
        } else if (code === 0 && /No title found\./.test(outputLog)){
