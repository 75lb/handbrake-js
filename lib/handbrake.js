...
/**
Runs HandbrakeCLI with the supplied [options](https://trac.handbrake.fr/wiki/CLIGuide) calling the supplied
callback on completion. 
@method exec
@param {Object|Thing|Array} options [Options](https://trac.handbrake.fr/wiki/CLIGuide) to pass directly to HandbrakeCLI
@param {Function} [onComplete] If passed, `onComplete(err, stdout, stderr)` will be called on completion, `stdout` and 
`stderr` being strings containing the HandbrakeCLI output.

The exec method is best suited for short duration tasks: 
@example
...
HandbrakeProcess.prototype.run = function(options){
    ...
    if (!config.valid){
        config.validationMessages.forEach(function(prop){
            prop.validationMessages.forEach(function(msg){
                self.emit("invalid", prop.property + ":\t" + msg);
            });
        });
        return;
    }
    ...
    handle.stdout.on("data", function(data){
        ...
        if (/Encoding:/.test(data)){
            ...
            var progress = {
                percentComplete: match[3] || 0,
                fps: +match[5] || 0,
                avgFps: +match[6] || 0,
                eta: match[7] || 0,
                task: match[2] || "Encoding"
            };
...
/**
Options [Thing](https://github.com/75lb/nature) describing all valid Handbrake option names, types and values.
