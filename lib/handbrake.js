...
function HandbrakeProcess(){
    ...
    this.progressStream = new stream.Transform();
    
    this.progressStream._transform = function(chunk, encoding, done){
        ...
        lines.forEach(function(line){
            if (progress.pattern.test(line)){
                ...
                */
                ...
                self.progressStream.push(JSON.stringify(progressObject));
...
HandbrakeProcess.prototype.run = function(options){
    ...
    When encoding (`--input` has been supplied) all text on stdout regards progress,
    so pipe it to the progressStream
    */
    var handle = cp.spawn(_HandbrakeCLIPath, config.toArray());
    handle.stderr.pipe(this.outputStream, { end: false });
    if (config.input){
        handle.stdout.pipe(this.progressStream);
    } else {
