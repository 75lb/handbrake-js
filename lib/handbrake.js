...
function HandbrakeProcess(){
    ...
    this.infoTransform._transform = function(chunk, enc, done){
        ...
        infoBuffer += chunkString;

        while((match = infoBuffer.match(/(.*\r?\n)|(\r.*)/)) !== null){
            var line = match[0].replace(/\r?\n/, "");
            self.infoTransform.push(new Buffer(JSON.stringify({ info: line }), "ascii"));
            self.emit("output", line);
            infoBuffer = infoBuffer.replace(match[0] ,"");
        }
        done();
    };
        ...
        lines.forEach(function(line){
            if (progress.pattern.test(line)){
                ...
                */
                ...
                self.progressTransform.push(new Buffer(JSON.stringify({ progress: progressObject }), "ascii"));
