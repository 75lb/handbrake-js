...
HandbrakeProcess.prototype.run = function(options){
    ...
    /*
    Spawn HandbrakeCLI, piping both stdout and stderr to the aggregated outputStream.
    While encoding (`--input` has been supplied) all text on stdout regards progress,
    so pipe it via the progressTransform.
    */
    var handle = cp.spawn(_HandbrakeCLIPath, config.toArray());
    handle.stderr.pipe(this.infoTransform, { end: false }).pipe(this.outputStream, { end: false });
    if (config.input){
        handle.stdout.pipe(this.progressTransform).pipe(this.outputStream, { end: false });
    } else {
        handle.stdout.pipe(this.infoTransform, { end: false }).pipe(this.outputStream, { end: false });
    }
